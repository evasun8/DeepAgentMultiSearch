...
const fetchFiles = async () => {
  ...
  try {
    const res = await axios.get('http://localhost:8001/api/files', {
      params: { path: currentSessionPath.value }
    })
    if (res.data.files) {
      fileList.value = res.data.files.map((f: any) => ({
        ...f,
        url: `http://localhost:8001/api/download?path=${encodeURIComponent(f.path)}`
      }))
    }
  } catch (e) {
...
const connectWebSocket = () => {
  const ws = new WebSocket(`ws://localhost:8001/ws/${currentThreadId.value}`)
...
const handleSocketMessage = (data: any) => {
  ...
  if (event === 'session_created') {
    ...
    if (parts.length > 1) {
      currentSessionUrl.value = `http://localhost:8001/outputs/${parts[1].replace(/\\/g, '/')}`
    }
    ...
    if (lastAiMsg) {
      ...
      lastAiMsg.logs.push({
        type: 'tool',
        title: `Use tool: ${eventData.tool_name}...`,
    ...
    if (currentSessionPath.value) {
     ...
     if (lastAiMsg) {
      ...
      lastAiMsg.logs.push({
        type: 'agent',
        title: `using assistant: ${eventData.assistant_name}...`,
        details: eventData.args,
        timestamp: new Date().toLocaleTimeString()
      })
    }
...
const sendMessage = async () => {
  ...
  try {
    ...
    const res = await axios.post('http://127.0.0.1:8001/api/task', payload)
    ...
    <main class="main-content" :class="{ 'centered-layout': isWelcomeScreen }">
      ...
      <div v-else class="chat-scroll-area">
        <div class="chat-container">
          <div v-for="(msg, index) in messageList" :key="index" class="message-wrapper" :class="msg.role">
